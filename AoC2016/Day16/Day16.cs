...
namespace Day16
{
    class Program
    {
        private Program(string inputFile, bool part1)
        {
            ...
            if (lines.Length != 1)
            {
                throw new InvalidProgramException($"Bad input data expected a single line {lines.Length}");
            }
            var input = lines[0];
            if (part1)
            {
        ...
        public static string DragonCurve(string input)
        {
            ...
            //In "b", replace all instances of 0 with 1 and all 1s with 0.
            //The resulting data is "a", then a single 0, then "b".
            var chars = new char[input.Length * 2 + 1];
            for (var i = 0; i < input.Length; ++i)
            {
                var c = input[i];
                chars[i] = c;
                var bPos = input.Length * 2 - i;
                var bChar = (c == '1') ? '0' : '1';
                chars[bPos] = bChar;
            }
        ...
        public static string Checksum(string input)
        {
            ...
            {
                var chars = new char[crc.Length / 2];
                for (var i = 0; i < crc.Length; i += 2)
                {
                    var c1 = crc[i];
                    var c2 = crc[i + 1];
                    if (c1 == c2)
                ...
                crc = new string(chars);
            };
            return crc;
        }

        public static string FillDisk(string start, int diskSize)
        {
            var text = start;
            while (text.Length < diskSize)
            {
                text = DragonCurve(text);
            }
            text = text.Substring(0, diskSize);
